...
const Dashboard = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            // Fault-tolerant individual fetching
            ...
            const catsData = await safeFetch('/vendor/categories', []);
            const itemsData = await safeFetch('/vendor/menu-items', []);
            const custData = await safeFetch('/vendor/customers', { customers: [] });
            const subData = await safeFetch('/vendor/subscription', { subscription: null });
            const invData = await safeFetch('/vendor/inventory', []);
            const ordersData = await safeFetch('/vendor/orders', { orders: [] });
            const settingsData = await safeFetch('/vendor/settings', { settings: { shopName: '', name: '', upiId: '' } });
            ...
            setError(`Critical failure loading dashboard: ${err.message}`);
        } finally {
            setLoading(false);
        }
    };
                                                ...
                                                menuItems.map(item => (
                                                    <div key={item._id} className="bg-white border border-secondary-100 rounded-2xl overflow-hidden premium-shadow-hover flex flex-col h-full group">
                                                        ...
                                                        <div className="p-5 flex flex-col flex-1 relative">
                                                            ...
                                                            <p className="text-sm text-secondary-500 font-medium line-clamp-2 flex-1 mb-2">
                                                                {item.description || <span className="italic opacity-50">No description provided</span>}
                                                            </p>

                                                            {item.inventoryItemId && (
                                                                <div className="text-[10px] font-black tracking-widest uppercase text-primary-600 bg-primary-50 border border-primary-100 px-2 py-1 rounded-md mb-4 w-max shadow-sm">
                                                                    Stock Map: {inventoryItems.find(i => i._id === item.inventoryItemId)?.itemName || 'Unknown'}
                                                                </div>
                                                            )}
